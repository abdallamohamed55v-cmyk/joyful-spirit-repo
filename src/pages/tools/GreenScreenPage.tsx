...
const GreenScreenPage = () => {
  ...
  if ((stage as string) === "landing") {
    ...
    return (
      ...
      headline="Remove"
      accent="the green."
      description={`Upload a green-screen clip — clean alpha matte in seconds.`}
      heroImage={heroImg}
      cost={2}
      accept="video/*"
      resultType="video"
      ...
      <div className="h-[100dvh] w-full flex flex-col bg-paper text-ink overflow-hidden">
        <header className="h-14 shrink-0 flex items-center justify-between px-6 border-b border-ink/10 bg-paper/90 backdrop-blur">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="font-sora text-sm font-semibold text-ink leading-tight">Green Screen Studio</div>
              <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-ink/40">Remove the green</div>
            </div>
          </div>
          {resultUrl && (
            <a
        ...
        <div className="flex-1 min-h-0 grid grid-cols-[280px_1fr_340px]">
          ...
          <main className="relative overflow-hidden bg-[radial-gradient(ellipse_at_top,_hsl(var(--primary)/0.06),_transparent_60%)] flex items-center justify-center p-10">
            <div
              className="relative w-full max-w-[640px] aspect-video rounded-3xl bg-white border border-ink/10 shadow-[0_40px_80px_-30px_rgba(0,0,0,0.25)] overflow-hidden"
              style={
                stage === "result" && bg === "transparent"
                  ? {
                      backgroundImage:
              ...
              <AnimatePresence mode="wait">
                {stage === "generating" && video && (
                  <motion.div key="gen" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0">
                    ...
                    <motion.div
                      ...
                      className="absolute inset-x-0 h-1/2 pointer-events-none bg-gradient-to-b from-transparent via-emerald-400/40 to-transparent blur-md"
                    />
                ...
                {stage === "compose" && (
                  video ? (
                    ...
                    <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-ink/40 p-8 text-center">
                      <Film className="w-8 h-8" />
                      <div className="font-sora font-semibold text-sm text-ink/70">Upload a green-screen clip</div>
                      <div className="font-manrope text-xs text-ink/40">Pick a new background on the right</div>
                    </motion.div>
                  )
                )}
              </AnimatePresence>
          ...
          <aside className="border-l border-ink/10 bg-ink/[0.02] flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              <div>
                <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-ink/40 mb-2">Background</div>
                  ...
                  {BG_MODES.map((m) => {
                    ...
                    return (
                      <button
                        key={m.id}
                        onClick={() => setBg(m.id)}
                        className={`px-3 py-3 rounded-xl text-xs font-sora font-semibold border-2 transition text-left ${
              ...
              <div className="text-[11px] font-manrope text-ink/40 leading-relaxed">
                We use a neural matting model to remove the green and replace it with your chosen background.
              </div>
            ...
            <div className="p-6 pt-3 border-t border-ink/10 bg-paper/60 backdrop-blur">
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  {stage === "generating" ? "Processing…" : "Remove background"}
                </span>
  ...
  return (
    <div className="h-[100dvh] flex flex-col overflow-hidden relative bg-gradient-to-b from-zinc-950 via-black to-zinc-950">
      ...
      <button
        ...
        className={`fixed top-[calc(env(safe-area-inset-top)+1rem)] left-3 z-30 w-11 h-11 flex items-center justify-center rounded-full text-white ${GLASS_PILL}`}
        aria-label="Back"
      >
