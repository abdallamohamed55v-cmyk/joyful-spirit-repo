      ...
      {!inline && (
        <div className="rounded-xl border border-border/50 bg-background/40 backdrop-blur p-3 overflow-hidden flex flex-col">
          ...
          <div className="flex gap-2 mt-2">
            {isRunning && (
              <Button size="sm" variant="outline" className="flex-1" onClick={() => stopOperatorRun(runId)}>
                <Square className="w-3 h-3 mr-1" /> Stop
              </Button>
            )}
            <Button size="sm" variant="ghost" className="flex-1" onClick={onClose}>Close</Button>
          </div>
        </div>
      )}
      ...
      <div className={inline ? "rounded-xl border border-border/40 p-3 flex flex-col min-h-[300px]" : "rounded-xl border border-border/50 bg-background/40 backdrop-blur p-3 overflow-hidden flex flex-col"}>
        ...
        {inline && isRunning && (
          <div className="flex gap-2 mt-2 pt-2 border-t border-border/30">
            <Button size="sm" variant="outline" className="flex-1 h-8" onClick={() => stopOperatorRun(runId)}>
              <Square className="w-3 h-3 mr-1" /> Stop
            </Button>
          </div>
        )}
          ...
          <TabsContent value="app" className="flex-1 mt-2">
            {run?.published_url ? (
              ...
              <div className="h-full min-h-[240px] flex flex-col items-center justify-center gap-3 text-center p-4">
                <div className="text-sm font-medium text-foreground">Project is being built in Megsy Code</div>
                <Link to={`/build/${run.project_id}/chat`} className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground hover:opacity-90 transition-opacity">
            ...
            {screenshots.length > 0 && (
              <div className="space-y-1">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {screenshots.map((s) => (
                    <button key={s.id} type="button" onClick={() => setPreviewImageUrl(s.url!)} className="text-left">
                      <img src={s.url!} alt="Megsy OS screenshot" className="w-full h-20 object-cover rounded border border-border/40 hover:opacity-80" />
                    </button>
                  ))}
                </div>
              </div>
